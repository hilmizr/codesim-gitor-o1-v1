...
tokenizer = AutoTokenizer.from_pretrained("microsoft/unixcoder-base")  # Updated tokenizer
model = AutoModel.from_pretrained("microsoft/unixcoder-base")          # Updated model
model.eval()  # Set model to evaluation mode

def get_unixcoder_embedding(code_string):
    """
    Generates a UniXCoder embedding for the given code string using mean pooling.
...
def get_embedding(file_path, embedding_base_path, relative_path, ast_base_path, ast_relative_path, use_json=True):
    ...
    if os.path.exists(embedding_path):
        ...
        try:
            ...
            return None
        # Generate embedding
        embedding = get_unixcoder_embedding(code)  # Updated function call
        # Save embedding
...
def main():
    ...
    # Updated path to reflect UniXCoder
    embedding_base_path = os.path.join(os.getcwd(), "embeddings", "AST", "UniXCoder", current_datetime)
