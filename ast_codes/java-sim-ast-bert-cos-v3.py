...
import json
import jsonpickle
from transformers import BertTokenizer, BertModel  # No change needed
import torch
...
def get_bert_embedding(code_string):
    """
    Generates a BERT embedding for the given code string using mean pooling.
...
def get_embedding(file_path, embedding_base_path, relative_path, ast_base_path, ast_relative_path, use_json=True):
    ...
    if os.path.exists(embedding_path):
        ...
        try:
            ...
            return None
        # Generate embedding
        embedding = get_bert_embedding(code)  # Updated function call
        # Save embedding
...
def plot_confusion_matrix(TP, FP, FN, TN, threshold):
    """
...
def main():
    ...
    # embeddings/AST/BERT/{datetime}/
    embedding_base_path = os.path.join(os.getcwd(), "embeddings", "AST", "BERT", current_datetime)
    ...
    plot_confusion_matrix(best_TP, best_FP, best_FN, best_TN, best_threshold)
