...
tokenizer = AutoTokenizer.from_pretrained("microsoft/codebert-base")  # Updated tokenizer
model = AutoModel.from_pretrained("microsoft/codebert-base")          # Updated model
model.eval()  # Set model to evaluation mode

def get_codebert_embedding(code_string):
    """
    Generates a CodeBERT embedding for the given code string using mean pooling across all token embeddings.
...
def parse_java_code(file_path):
    ...
    try:
        ...
        logging.error(f"Syntax error while parsing '{file_path}': {e}")
        return None
    except Exception as e:
...
def save_ast(ast, save_path):
    ...
    try:
        ...
        logging.error(f"Error saving AST to '{save_path}': {e}")
...
def load_ast(load_path):
    ...
    try:
        ...
        logging.error(f"Error loading AST from '{load_path}': {e}")
        return None
...
def get_embedding(file_path, embedding_base_path, relative_path, ast_base_path, ast_relative_path, use_json=True):
    ...
    if ast is None:
        logging.warning(f"Failed to obtain AST for '{file_path}'. Skipping embedding generation.")
        return None
    ...
    if os.path.exists(embedding_path):
        ...
        try:
            ...
            return None
        # Generate embedding
        embedding = get_codebert_embedding(code)  # Updated function call
        # Save embedding
...
def main():
    ...
    # embeddings/AST/CodeBERT/{datetime}/
    embedding_base_path = os.path.join(os.getcwd(), "embeddings", "AST", "CodeBERT", current_datetime)
