...
# Function to generate a GloVe embedding by averaging word vectors
def get_glove_embedding(model, tokens):
    """
    Generates a GloVe embedding by averaging word vectors.

    Args:
        model (KeyedVectors): Loaded GloVe model.
...
# Function to prepare and load GloVe embeddings
def prepare_glove_embeddings(glove_dir='./glove', word2vec_dir='./glove_word2vec'):
    """
    Converts and loads GloVe embeddings.

    Args:
        glove_dir (str): Directory containing GloVe files.
        word2vec_dir (str): Directory to save Word2Vec formatted files.

    Returns:
        KeyedVectors: Loaded GloVe vectors.
    """
    os.makedirs(word2vec_dir, exist_ok=True)
    glove_file = os.path.join(glove_dir, 'glove.6B.100d.txt')
    word2vec_file = os.path.join(word2vec_dir, 'glove.6B.100d.word2vec.txt')
...
def get_embedding(file_path, embedding_base_path, relative_path, glove_model, embedding_type='glove', run_id=''):
    """
    ...
    Args:
        ...
        glove_model (KeyedVectors): Loaded GloVe model.
        embedding_type (str): The type of embedding (e.g., 'glove').
    ...
    if os.path.exists(embedding_path):
        ...
        if embedding_type == 'glove':
            embedding = get_glove_embedding(glove_model, tokens)
        else:
...
def main():
    ...
    embedding_type = 'glove'  # Changed from 'word2vec' to 'glove'

    # Load GloVe model
    glove_model = prepare_glove_embeddings()

    # Collect all Java files for processing
    java_file_paths = []
    ...
    if not java_file_paths:
        print("No Java files found in the dataset to process.")
        return

    # Since we're using pre-trained GloVe, no need to preprocess all tokens for training
    # Proceed with similarity calculations as before
